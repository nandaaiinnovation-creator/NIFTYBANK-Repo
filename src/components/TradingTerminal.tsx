...
const BrokerConnectionBar: React.FC = () => {
    ...
    const StatusIndicator = () => {
        switch (status) {
            case 'connected': return <div className="w-3 h-3 bg-green-500 rounded-full animate-pulse" title="Connected"></div>;
            case 'connecting': return <div className="w-3 h-3 bg-yellow-500 rounded-full animate-pulse" title="Connecting..."></div>;
            case 'error': return <div className="w-3 h-3 bg-red-500 rounded-full" title="Error"></div>;
            default: return <div className="w-3 h-3 bg-gray-500 rounded-full" title="Disconnected"></div>;
        }
    };
...
interface MarketTick { price: number; time: string; }
...
const LiveSignalsContent: React.FC = () => {
  ...
  const handleNewSignal = useCallback((newSignal: Signal) => {
    ...
    if (priceRange.max > priceRange.min) {
        ...
        const chartSignal: ChartSignal = { ...newSignal, id: `${newSignal.time}-${newSignal.price}`, x: 98, y: Math.max(5, Math.min(95, y)), }
  ...
  const handleNewTick = useCallback((tick: MarketTick) => {
    setCandlesticks(prevCandles => {
      ...
      if (lastCandle.minute !== tickMinute) {
        const newCandle: Candle = { id: Date.now(), open: tick.price, high: tick.price, low: tick.price, close: tick.price, minute: tickMinute };
    ...
    <div className="bg-gray-800/50 rounded-lg shadow-xl p-6 border border-gray-700">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
            ...
            className={`px-6 py-2 rounded-md font-semibold transition-all duration-300 ease-in-out flex items-center justify-center gap-2 ${isReceiving ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-green-600 hover:bg-green-700 text-white'}`} >
            {isReceiving ? <><i className="fas fa-stop-circle animate-pulse"></i> Stop Feed</> : <><i className="fas fa-play-circle"></i> Start Feed</>}
        </button>
      </div>
          ...
          <div className="lg:col-span-1 h-full overflow-y-auto pr-2 bg-gray-900/50 p-4 rounded-lg border border-gray-700">
            ...
            {signals.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500 text-center">
                ...
                <p className="text-sm mt-2">{isReceiving ? 'Receiving live data...' : 'Press "Start Feed" to begin.'}</p>
              </div>
